...
// backend/node_modules/hono/dist/utils/buffer.js
...
// backend/node_modules/hono/dist/router/utils.js
...
// backend/node_modules/hono/dist/utils/cookie.js
...
// backend/node_modules/hono/dist/helper/cookie/index.js
...
var app = new Hono2();
var SESSION_COOKIE = "maguru_session";
...
async function createSession(c, user) {
  ...
  const token = await sign2(
    { sub: user.id, email: user.email, role: user.role, exp: Math.floor(Date.now() / 1e3) + SESSION_TTL },
    secret
  );
...
async function requireAuth(c, next) {
  ...
  try {
    c.set("session", await verify2(token, c.env.JWT_SECRET));
    await next();
  } catch {
...
app.post("/api/cars", requireAuth, async (c) => {
  ...
  await c.env.DB.prepare(`INSERT INTO cars
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(carId, car.title, car.make, car.model, car.year ?? null, car.mileage ?? null, car.price, car.transmission ?? null, car.fuel_type ?? null, car.body_type ?? null, car.color ?? null, car.engine_cc ?? null, car.condition ?? null, car.status || "Available", car.description ?? null, JSON.stringify(car.images || []), timestamp, timestamp).run();
  return c.json({ id: carId });
});
app.put("/api/cars/:id", requireAuth, async (c) => {
  const car = await c.req.json();
  const timestamp = now();
  const result = await c.env.DB.prepare(`UPDATE cars SET title=?, make=?, model=?, year=?, mileage=?, price=?, transmission=?, fuel_type=?, body_type=?, color=?, engine_cc=?, condition=?, status=?, description=?, images_json=?, updated_at=? WHERE id=?`).bind(car.title, car.make, car.model, car.year ?? null, car.mileage ?? null, car.price, car.transmission ?? null, car.fuel_type ?? null, car.body_type ?? null, car.color ?? null, car.engine_cc ?? null, car.condition ?? null, car.status || "Available", car.description ?? null, JSON.stringify(car.images || []), timestamp, c.req.param("id")).run();
...
app.post("/api/inquiries", async (c) => {
  ...
  const timestamp = now();
  const inquiryId = id();
  await c.env.DB.prepare(`INSERT INTO inquiries (id, name, email, phone, message, car_id, car_title, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'new', ?, ?)`).bind(inquiryId, inquiry.name, inquiry.email, inquiry.phone, inquiry.message, inquiry.carId || null, inquiry.carTitle || null, timestamp, timestamp).run();
...
app.post("/api/users", requireAuth, async (c) => {
  ...
  const timestamp = now();
  const userId = id();
  await c.env.DB.prepare("INSERT INTO users (id, name, email, password_hash, role, created_at, updated_at) VALUES (?, ?, ?, ?, 'admin', ?, ?)").bind(userId, user.name, user.email.toLowerCase(), await hashPassword(user.password), timestamp, timestamp).run();
...
app.put("/api/users/:id", requireAuth, async (c) => {
  ...
  const values = user.password ? [user.name, user.email.toLowerCase(), await hashPassword(user.password), timestamp, c.req.param("id")] : [user.name, user.email.toLowerCase(), timestamp, c.req.param("id")];
...
// .wrangler/tmp/bundle-kMtep6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kMtep6/middleware-loader.entry.ts
